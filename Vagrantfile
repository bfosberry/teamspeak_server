...
Vagrant.configure("2") do |config|
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "core-%02d" % i do |config|
      ...
      config.vm.provision :shell, :inline => "mv /tmp/Dockerfile /home/core/Dockerfile", :privileged => true
      config.vm.provision :shell, :inline => "mv /tmp/run.sh /home/core/run.sh", :privileged => true
    end
  end
end
